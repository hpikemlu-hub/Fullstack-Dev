...
export function Sidebar({ user, isOpen = true, isCollapsed = false, onClose, onToggleCollapse }: SidebarProps) {
  ...
  // Using new API endpoints instead of Supabase
                ...
                <div className="flex items-center gap-3 w-full">
                  <Avatar className="h-10 w-10">
                    <AvatarFallback className="bg-blue-100 text-blue-600 font-medium">
                      {getInitials((user as any).nama_lengkap || (user as any).username || (user as any).email || 'Unknown User')}
                    </AvatarFallback>
                  </Avatar>
                  <div className={cn("flex-1 min-w-0", isCollapsed && "lg:hidden")}>
                    <p className="text-sm font-medium text-gray-900 truncate text-left">
                      {(user as any).nama_lengkap || (user as any).username || (user as any).email || 'Unknown User'}
                    </p>
                    <p className="text-xs text-gray-500 truncate text-left">
                      {(user as any).jabatan || 'Staff'} • {(user as any).role === 'admin' ? 'Administrator' : 'User'}
                    </p>
                  </div>
                </div>
              </Button>
            ...
            <DropdownMenuContent className="w-56 mb-2" align="start" side="top">
              <DropdownMenuLabel className="font-normal">
                <div className="flex flex-col space-y-1">
                  <p className="text-sm font-medium leading-none">{(user as any).nama_lengkap || (user as any).username || (user as any).email || 'Unknown User'}</p>
                  <p className="text-xs leading-none text-muted-foreground">
                    {(user as any).jabatan || 'Staff'} • {(user as any).role?.charAt(0).toUpperCase() + (user as any).role?.slice(1) || 'User'}
                  </p>
                </div>
              </DropdownMenuLabel>
              <DropdownMenuSeparator />
