...
export function ProfessionalEmployeeTable({ 
  ...
  const hierarchyGroups = groupEmployeesByHierarchy(employees);
  const directors = hierarchyGroups.find(g => g.level === 1)?.employees || [];
  const coordinators = hierarchyGroups.find(g => g.level === 2)?.employees || [];
  const staff = hierarchyGroups.filter(g => g.level > 2).flatMap(g => g.employees) || [];
