...
export async function fetchUniqueLocations(): Promise<string[]> {
  ...
  const locationSet = new Set(data.map(item => item.location).filter(Boolean) as string[]);
...
export async function fetchUniqueDipaCodes(): Promise<string[]> {
  ...
  const dipaSet = new Set(data.map(item => item.dipa).filter(Boolean) as string[]);
  const dipaCodes = Array.from(dipaSet);
  return dipaCodes;
}
...
export async function fetchUniqueCategories(): Promise<string[]> {
  try {
    ...
    const categorySet = new Set((data || []).map((item: any) => item.event_type).filter(Boolean) as string[]);
